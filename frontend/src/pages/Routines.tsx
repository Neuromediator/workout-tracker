  ...
  return (
    <div className="space-y-8">
      ...
      <div className="flex items-center justify-between">
        <div>
          <h2 className="font-heading text-xl font-bold tracking-tight">{t('routines.title')}</h2>
          <p className="text-sm text-muted-foreground">
            {custom.length} {t('routines.customCount')}, {templates.length} {t('routines.templates')}
          </p>
        </div>
        <Button size="sm" onClick={onNew} className="bg-warm text-warm-foreground hover:bg-warm/90">
          <Plus className="mr-1 h-4 w-4" /> {t('routines.newRoutine')}
        </Button>
      </div>
      ...
      {custom.length === 0 && templates.length === 0 ? (
        <div className="flex flex-col items-center justify-center rounded-xl border border-dashed border-border/50 py-16 text-muted-foreground">
          ...
          <p className="font-medium">{t('routines.noRoutines')}</p>
          <p className="mt-1 text-sm">{t('routines.createOrTemplate')}</p>
        </div>
      ) : (
        <>
          {/* Custom routines */}
